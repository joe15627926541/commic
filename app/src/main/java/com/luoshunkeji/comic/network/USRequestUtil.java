...
import com.luoshunkeji.comic.utils.ObjectUtil;
...
public class USRequestUtil {
	...
	private static void checkDB(Context context) {
		if (rqdb == null) {
			...
			// 创建数据库
			rqdb = context.openOrCreateDatabase(DB_NANME, Application.MODE_PRIVATE, null);
			try {
				// 创建缓存数据库表
				rqdb.execSQL("create table if not exists " + TABLE_NAME + "  (" + TABLE_ID + " integer PRIMARY KEY," + TABLE_METHOD + " integer," + TABLE_URL + " text," + TABLE_POSTDATA + " BLOB,"
						+ TABLE_HEADDATA + " BLOB)");
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static boolean del(Context context, int id) {
		...
		checkDB(context);
		try {
			rqdb.delete(TABLE_NAME, TABLE_ID + "=?", new String[] { id + "" });
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static ArrayList<RqData> listall(Context context) {
		...
		try {
			Cursor sur = rqdb.rawQuery("select * from " + TABLE_NAME + " ORDER BY '" + TABLE_ID + "' asc", null);// 上到下开始请求
			if (sur != null) {

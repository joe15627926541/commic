...
public class CookieUtil {
		...
		if (mdb == null) {
			if (context == null)
				throw new IllegalStateException(
						"application is null,DbUtil cannot create");
			try {
				// 创建数据库
				mdb = context.openOrCreateDatabase(DB_NAME,
						Application.MODE_PRIVATE, null);
				// 创建据库表
				mdb.execSQL("create table if not exists " + TABLE_NAME + "  ("
						+ TABLE_ID + " integer PRIMARY KEY," + TABLE_URL
						+ " text," + TABLE_KEY + " text," + TABLE_VALUE
						+ " text," + TABLE_TIMEOUT + " bigint)");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 * @return
	 */
	public static boolean setcookie(Context context, String url,
                                    String setcookieStr) {
	 ...
	 * 设置数据
	 */
	public static boolean setValue(Context context, String url, String key,
	...
	public static String getCookie(Context context, String url) {
		...
		try {
			Cursor sur = mdb.rawQuery("select * from " + TABLE_NAME + " where "
					+ TABLE_URL + "='" + url + "' and " + TABLE_TIMEOUT + ">"
					+ new Date().getTime(), null);
			if (sur != null) {
				while (sur.moveToNext()) {
					var.append(sur.getString(sur.getColumnIndex(TABLE_KEY))
							+ "="
							+ sur.getString(sur.getColumnIndex(TABLE_VALUE))
							+ "; ");
				}
				sur.close();
			}
	...
	public static boolean delKey(Context context, String url, String key) {
		checkDB(context);
		try {
			mdb.execSQL("delete from " + TABLE_NAME + " where " + TABLE_KEY
					+ "='" + key + "'");
			return true;
		} catch (Exception e) {
			e.printStackTrace();
